RSpec.describe "Api::Clients::Buildings", type: :request do
  subject { send(http_method, url) }
  
  let(:http_method) { :get }
  let(:url) { api_client_buildings_path(client_id: client.id) }
  ...
  # describe "POST /api/clients/:client_id/buildings" do
  ...
  # describe "PUT /api/clients/:client_id/buildings/:id" do
